...
bool WebSockets::handleWebsocketWaitFor(WSclient_t * client, size_t size) {
    ...
    readCb(client, &client->cWsHeader[client->cWsRXsize], (size - client->cWsRXsize),  readWaitcallBack );
...
void WebSockets::readWaitcallBack(WebSockets* ws,size_t size,WSclient_t * client, bool ok)
{
  DEBUG_WEBSOCKETS("[WS][%d][handleWebsocketWaitFor][readCb] size: %d ok: %d\n", client->num, size, ok);
  if(ok) {
	  client->cWsRXsize = size;
	  ws->handleWebsocketCb(client);
	  } else {
	  DEBUG_WEBSOCKETS("[WS][%d][readCb] failed.\n", client->num);
	  client->cWsRXsize = 0;
	  // timeout or error
	  ws->clientDisconnect(client, 1002);
  }
...
void WebSockets::handleWebsocketCb(WSclient_t * client) {
    ...
    if(header->payloadLen > 0) {
        ...
        readCb(client, payload, header->payloadLen, readWaitcallBack);//,std::bind(&WebSockets::handleWebsocketPayloadCb, this, std::placeholders::_1, std::placeholders::_2, payload));
    } else {
